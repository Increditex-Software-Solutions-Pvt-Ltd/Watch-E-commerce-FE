...
import { INCREMENT_QUANTITY, DECREMENT_QUANTITY, REMOVE_FROM_CART } from '../context/CartContext';
...
const Shoppingcart = () => {
  ...
  return (
                    ...
                    <div className="item-details">
                      ...
                      <div className="d-flex flex-row align-items-center">
                        <div className="rounded-3 counterbox overflow-hidden">
                          <button className="counterbtn border-0 minusbtn" style={{ fontSize: "17px" }} onClick={() => item.quantity > 1 ? decrementQuantity(item.cartId) : handleOpen(item)}>
                            <i className="bi bi-dash normal"></i>
                          </button>
                          <span className='px-3 bg-white' style={{ fontSize: '16px' }}>{item.quantity}</span>
                          <button className="counterbtn bg-white border-0" style={{ fontSize: "17px" }} onClick={() => incrementQuantity(item.cartId)}>
        ...
        <div className="card rounded-2 position-sticky" style={{ top: "130px" }}>
          ...
          <div className="card-body">
            <div className="d-flex justify-content-between" style={{ fontSize: "14px" }}>
              ...
              <span className='text-secondary' style={{ fontWeight: "500" }}>₹ {cartState.total}</span>
            </div>
          ...
          <div className="card-footer bg-white p-3">
            <div className="d-flex justify-content-between">
              ...
              <h6 className="text-dark">₹ {cartState.total}</h6>
            </div>
            <div className="mt-2">
    ...
    <ConfirmRemove open={open} handleClose={handleClose} handleConfirmRemove={() => removeFromCart(currentProduct.cartId)} />
  </div>
  );
};
